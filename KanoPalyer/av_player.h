...
public:
    void soundTouuch(uint8_t*inPcmBuf,int inPcmBufsize,uint channels,uint sampleRate,int bytePerSample,double tempo,double pitch);
public:
    enum PlayState
    {
        AV_STOPPED,
...
//    double m_videoPts;
    ...
    soundtouch::SoundTouch *s_touch;//soundtouch实例

    int16_t *touch_buffer=nullptr;//soundtouch缓存
    uint8_t *dataBuf= nullptr;
    uint32_t dataBufSize;
    int  bufPos=0;//已经处理后数据长度

    uint8_t* m_pixels[4];//m_pixels[0]即pFrame->data[0] = yuvData[0];//Y数据
    //pFrame->data[1] = yuvData[1];//U数据    pFrame->data[2] = yuvData[2];//V数据


    int m_pitch[4];//即：pFrame->linesize[0] = linesize[0];  // Y数据行长字节数, 720
