...
#include <QDebug>
#include "KanoPalyer/KanoPlayer.h"
#include <QMouseEvent>
#define VERTEXIN 0
#define TEXTUREIN 1
...
KanoOpenGLWidget::KanoOpenGLWidget(QWidget *parent)
        : QOpenGLWidget(parent),m_isDoubleClick(0)
{
...
//void KanoOpenGLWidget::slotShowYuv(uchar *ptr, uint width, uint height)
//{
//    m_yuvPtr = ptr;
...
void KanoOpenGLWidget::onShowYUV(QSharedPointer<YUV420Frame> frame)
{
    if(!frame)
        qDebug()<<"null frame!\n";
    if(m_frame)
...
void KanoOpenGLWidget::initializeGL()
{
...
void KanoOpenGLWidget::mouseReleaseEvent(QMouseEvent *event)
{
//    if(!m_timer.isActive())
//        m_timer.start();
...
void KanoOpenGLWidget::mouseDoubleClickEvent(QMouseEvent *event)
{
    m_isDoubleClick=1;
